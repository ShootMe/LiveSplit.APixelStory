		...
		public static T Read<T>(this Process targetProcess, IntPtr address, params int[] offsets) {
			...
			try {
				for (int i = 0; i < offsets.Length - 1; i++) {
					SafeNativeMethods.ReadProcessMemory(targetProcess.Handle, address + offsets[i], buffer, 4, out bytesRead);
					address = (IntPtr)BitConverter.ToInt32(buffer, 0);
				}
				int last = offsets.Length > 0 ? offsets[offsets.Length - 1] : 0;
				if (typeof(T) == typeof(int)) {
					SafeNativeMethods.ReadProcessMemory(targetProcess.Handle, address + last, buffer, 4, out bytesRead);
					return (T)(object)BitConverter.ToInt32(buffer, 0);
				} else if (typeof(T) == typeof(long)) {
					SafeNativeMethods.ReadProcessMemory(targetProcess.Handle, address + last, buffer, 8, out bytesRead);
					return (T)(object)BitConverter.ToInt64(buffer, 0);
				} else if (typeof(T) == typeof(byte)) {
					SafeNativeMethods.ReadProcessMemory(targetProcess.Handle, address + last, buffer, 1, out bytesRead);
					buffer[1] = 0;
					return (T)(object)(byte)BitConverter.ToInt16(buffer, 0);
				} else if (typeof(T) == typeof(short)) {
					SafeNativeMethods.ReadProcessMemory(targetProcess.Handle, address + last, buffer, 2, out bytesRead);
					return (T)(object)BitConverter.ToInt64(buffer, 0);
