	...
	public class APixelStoryComponent : IComponent {
		...
		public void GetValues() {
			...
			switch (currentSplit) {
				...
				//Generation 2 & Valley
				case 2: shouldSplit = mem.GetScene() == "UpgradeRoom" || mem.GetSolveCount("Dispenser1") == 1; break;
				//Registry & Generation 2
				case 3: shouldSplit = mem.GetMapLevelName() == "Gen2Core1Narrative" || (mem.GetScene() == "UpgradeRoom" && mem.GetSolveCount("Dispenser2") == 1); break;
				//Generation 3 & Registry
				case 4: shouldSplit = (mem.GetScene() == "UpgradeRoom" && mem.GetSolveCount("Dispenser2") == 0) || (mem.GetMapLevelName() == "Gen2Core1Narrative" && mem.GetSolveCount("Dispenser3") == 1); break;
				//Recycle Bin & Generation 3
				case 5: shouldSplit = mem.GetMapLevelName() == "Gen3Core1Narrative" || (shouldSplit = mem.GetScene() == "UpgradeRoom" && mem.GetSolveCount("Dispenser4") == 1); break;
				//Generation 4 & Recycle Bin
				case 6: shouldSplit = (mem.GetScene() == "UpgradeRoom" && mem.GetSolveCount("Dispenser4") == 0) || (mem.GetMapLevelName() == "Gen3Core1Narrative" && mem.GetSolveCount("Dispenser4") == 1); break;
				//Core Ending & Forest Complete
				case 7: shouldSplit = (previousFadebox == FadeboxState.fadingGameIn && mem.GetMapLevelName() == "Gen4Core1" && mem.GetFadeboxState() == FadeboxState.fadingGameOut) || mem.GetSolveCount("Dispenser5") == 1; break;
				//Generation 4
				case 8: shouldSplit = mem.GetScene() == "UpgradeRoom" && mem.GetSolveCount("Dispenser6") == 1; break;
				//Core Ending
				case 9: shouldSplit = previousFadebox == FadeboxState.fadingGameIn && mem.GetMapLevelName() == "Gen4Core1" && mem.GetFadeboxState() == FadeboxState.fadingGameOut; break;
				//Ragequit
			...
			if (Model != null && Model.CurrentState.Run.Count == 15 && (Model.CurrentState.CurrentPhase == TimerPhase.Running || roomsCompleted < 15) && mem.IsDeathrun()) {
				...
				shouldSplit = previousScene != scene || (roomsCompleted == 15 && (DateTime.Now - finalDoor.Value).TotalSeconds >= 4.4);
		...
		private void LogValues() {
			if (lastLogCheck == 0) {
				hasLog = File.Exists("_APixelStory.log");
				lastLogCheck = 300;
			}
			lastLogCheck--;
